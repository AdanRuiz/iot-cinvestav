				...
				if (SENSORS.get(i).getSensor_type().equals(type))
					{
						...
						entityService.getSensorData(SENSORS.get(i).getId(), 1,
								new AsyncCallback<List<MeasureDTO>>() {
									...
									public void onSuccess(final List<MeasureDTO> result)
									{
								 	  ...
								 	  ListHandler<MeasureDTO> sortHandler = new ListHandler<MeasureDTO>(dataProvider.getList());
							  		   ...
							  		   sortHandler.setComparator(dateColumn,new Comparator<MeasureDTO>() {
							  			  @Override
							 		    ...
							 		    tablePhotos.getColumnSortList().push(dateColumn);
							 		    formPanel.add(tablePhotos);
